...
class ShuffleNetV2BlockSearched(nn.Module):
    def __init__(self, prefix, in_channels, out_channels, stride, base_mid_channels, i_th, architecture):
        ...
        self.stride = stride
        if self.stride == 2:
            self.conv = Blocks[op](prefix + '_' + op, in_channels,
                                   out_channels - in_channels, base_mid_channels, stride, True)
        else:
            self.conv = Blocks[op](prefix + '_' + op, in_channels //
                                   2, out_channels // 2, base_mid_channels, stride, True)
        if stride > 1:
...
class ShuffleNetV2DetNAS(nn.Module):
    def __init__(self, n_class=1000, model_size='VOC_FPN_300M'):
        ...
        if model_size == 'COCO_FPN_3.8G':
            ...
            architecture = [2, 1, 2, 0, 2, 1, 1, 2,
                            3, 3, 1, 3, 0, 0, 3, 1, 3, 1, 3, 2]
            stage_repeats = [4, 4, 8, 4]
            stage_out_channels = [-1, 16, 64, 160, 320, 640, 1024]
        elif model_size == 'COCO_RetinaNet_300M':
            architecture = [2, 3, 1, 1, 3, 2, 1, 3,
                            3, 1, 1, 1, 3, 3, 2, 0, 3, 3, 3, 3]
            stage_repeats = [4, 4, 8, 4]
            stage_out_channels = [-1, 16, 64, 160, 320, 640, 1024]
        elif model_size == 'VOC_FPN_300M':
            architecture = [2, 1, 0, 3, 1, 3, 0, 3,
                            2, 0, 1, 1, 3, 3, 3, 3, 3, 3, 3, 1]
            stage_repeats = [4, 4, 8, 4]
            stage_out_channels = [-1, 16, 64, 160, 320, 640, 1024]
        elif model_size == 'VOC_RetinaNet_300M':
            architecture = [1, 3, 0, 0, 2, 3, 3, 3,
                            2, 3, 3, 3, 3, 2, 2, 0, 2, 3, 1, 1]
            stage_repeats = [4, 4, 8, 4]
            stage_out_channels = [-1, 16, 64, 160, 320, 640, 1024]
        else:
        ...
        self.first_conv = ConvBNReLU(
            in_channel=3, out_channel=stage_out_channels[1], k_size=3, stride=2, padding=1, gaussian_init=True)
        ...
        self.last_conv = ConvBNReLU(
            in_channel=in_channels, out_channel=stage_out_channels[-1], k_size=1, stride=1, padding=0)
