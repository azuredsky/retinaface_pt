...
parser.add_argument('-v', '--video', default='', help='video file to inference')
...
if __name__ == '__main__':
    ...
    if not cap.isOpened():
        print('can not open this video file: {}'.format(v_f))
    ...
    while (cap.isOpened()):
        ...
        if ok:
            ...
            cost = time.time() - tic
            print('net forward time: {:.4f}'.format(cost))
            ...
            cv2.putText(img_raw, "FPS: {:.3f}".format(1/cost), (10, 30), cv2.FONT_HERSHEY_DUPLEX, 0.7, (0, 255, 0))
            # save image
